...
- **捕获**：默认 `Ctrl+Alt+Space` 或托盘「快速收集」呼出快速收集窗，`todo:` 前缀进入收件箱、`note:` 创建便签、无前缀默认 Todo；主面板内可直接创建 Todo / 便签；GitHub 视图的 PR / Issue 行可一键「转为 Todo」（默认进收件箱并携带来源）。
