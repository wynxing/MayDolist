...
- **动作（快捷键 `1`-`5`，全部复用现有 command / service，不新增写路径）**：
  ...
  - `2` 稍后做：`todo_update_item` 写入 `dueDate=今天+3 天`；
...
- **单元测试**：动作映射、稳定 id 光标推进 / 校正、剩余计数与完成态判定、日期计算（今天 / +3 天，跨月跨年）均为纯函数并有单测（`src/triage.ts` + `src/triage.test.ts`）。
